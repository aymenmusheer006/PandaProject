...
x=1
while(x<btlen):
    if(str(df['BATCH'][x]) not in btchnos):
        btchnos.append(str(df['BATCH'][x]))
    x+=1
def cmd():
    ...
    if(avg==0):
        ...
        temp=df['Date Time'][0]+timedelta(seconds=int(avg))
        x=1
        lslen=len(df['Date Time'])
        tp1=()
        tp1=list(tp1)
        ...
        while(x<lslen):
            if(df['Date Time'][x]<=temp):
                ...
                tp2.append(tp1.mean(axis=0))
                temp+=timedelta(seconds=int(avg))
    ...
    with open(fdt+'.csv', 'w') as csv_file:
        ...
        writer.writerow(["batch of 20202020"])
...
opt=tk.IntVar()
tk.Radiobutton(m, text='1 min' ,variable=avg1, value=60).grid(row=3,column=0)
tk.Radiobutton(m, text='10 min', variable=avg1, value=600).grid(row=3,column=1)
tk.Radiobutton(m, text='30 min', variable=avg1, value=1800).grid(row=3,column=2)
tk.Radiobutton(m, text='60 min', variable=avg1, value=3600).grid(row=3,column=3)
tk.Radiobutton(m, text='None', variable=avg1, value=0).grid(row=3,column=4)
tk.Radiobutton(m, text='PDF' ,variable=opt, value=1).grid(row=8,column=0)
tk.Radiobutton(m, text='XLSX', variable=opt, value=2).grid(row=8,column=1)
tk.Radiobutton(m, text='CSV', variable=opt, value=3).grid(row=8,column=2)
...
button = tk.Button(m, text='Print', width=25, command=cmd).grid(row=10,column=1,columnspan=2)
tk.Label(m, text='').grid(row=11,column=0)
